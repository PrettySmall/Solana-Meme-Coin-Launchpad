...
const Token2022LaunchPage = () => {
    const { currentProject } = useContext(AppContext);
    const [selectedProject, setSelectedProject] = useState((currentProject.platform != 'token-2022' || currentProject.paymentId == 0) ? { name: "New Project" } : { ...currentProject });
    ...
    useEffect(() => {
        if (step == 0)
            setSelectedProject((currentProject.platform != 'token-2022' || currentProject.paymentId == 0) ? { name: "New Project" } : { ...currentProject });
        // setStep(0);
                ...
                <div className="w-full h-[30%] grow px-10 pb-10 flex gap-12">
                    <ol class="relative min-w-[200px] h-full flex flex-col justify-between text-gray-500 border-e-2 border-gray-normal dark:border-gray-700 dark:text-gray-400">
                        ...
                        <li class="me-6 cursor-pointer hover:bg-gray-weight" onClick={() => { confirmedStep >= 3 && setStep(3) }}>
                            ...
                            <h3 class={`font-medium leading-tight text-sm ${confirmedStep == 3 ? "text-white" : confirmedStep > 3 && "text-green-normal"} ${step == 3 && "brightness-125"}`}>Bundle</h3>
                        </li>
                    </ol>
                    <div className="w-full h-full flex flex-col gap-2">
                        ...
                        }
                        {
                            step == 3 && <div className="wizard-div overflow-auto p-6 flex flex-col items-center justify-center">
                                <Token2022Buy className={"w-full h-full"} />
                            </div>
                        }
                    </div>
                </div>
...
export default Token2022LaunchPage;
