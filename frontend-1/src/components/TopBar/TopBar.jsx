  ...
  const handleCopyInviteCode = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${ENV.SERVER_URL}/api/v1/user/invite-code`,
        {
          sigData,
          signingData,
        },
    ...
    <div className="sticky top-0 w-full h-12 px-4 flex items-center justify-between gap-10 bg-[#0d0e0f] shadow-xl z-10">
      ...
      <div className="h-full flex-[5] flex flex-row-reverse gap-3 items-center">
        ...
        <div className="h-full flex gap-3 items-center">
          <div
            className="w-fit h-fit p-2 rounded-full hover:bg-gray-highlight cursor-pointer"
          ...
          </div>
          {
            user && sigData && signingData && <>
              ...
              <div
                className={`h-fit flex font-medium text-center cursor-pointer px-4 py-1 rounded-md bg-white hover:bg-white/65 active:bg-white/85 text-black justify-center items-center`}
